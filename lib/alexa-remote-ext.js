...
class AlexaRemoteExt extends AlexaRemote
{
	...
	async updateExt() {
		...
		const warn = (what, error) => (error.message = `failed to initialise ${what}: "${error.message}"`, this.warnCallback(error));
		const handleNonCritical = (promise, prop, label) => promise
			.then(() => {
				this.errorMessagesExt[prop] = null;
			})
			.catch(error => {
				error.message = `failed to initialise ${label || prop}: "${error.message}"`;
		...
		await Promise.all([
			...
			handleNonCritical(this.initSmarthomeSimplifiedExt(), 'smarthome', 'smarthome simplified'),
			handleNonCritical(this.initSmarthomeColorsExt(), 'colors', 'smarthome colors'),
		]);

		const customerIds = {};
		for(const device of this.deviceByIdExt.values()) {
			const id = device.deviceOwnerCustomerId;
			customerIds[id] = (customerIds[id] || 0) + 1;
		}
	...
	async initSmarthomeSimplifiedExt() {
		...
		const [groups, entityByEntityId, deviceByApplianceId] = await Promise.all([
			...
			this.getSmarthomeDevicesPromise().then(response => {
				...
				//throw new Error('lmao you fucked up')
	...
	resetExt() {
		if (this.alexaWsMqtt) {
			this.alexaWsMqtt.on('error', (err) => console.log(err));
		}
