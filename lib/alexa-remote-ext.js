...
class AlexaRemoteExt extends AlexaRemote
{
	...
	async updateExt() {
		//const warn = (what => (error => (error.message = `failed to ${what}: ` + error.message, this.logWarn(error))));
		const warn = (what, error) => (error.message = `failed to initialise ${what}: "${error.message}"`, this.logWarn(error));
		...
		await handleNonCritical(this.initSmarthomeSimplifiedExt(), 'smarthome', 'smarthome simplified');
